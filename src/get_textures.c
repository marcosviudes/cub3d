...
/*   Updated: 2020/12/20 14:18:49 by mviudes          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		get_textures(t_config *config, t_mlx *mlx)
{
	...
	while (i < 5)
	{
		mlx->tex[i].img = mlx_xpm_file_to_image(mlx->mlx, tex_paths[i],
												&mlx->tex[i].img_width,
												&mlx->tex[i].img_height);
		if (!mlx->tex[i].img)
			error_exit("No se ha podido abrir la textura");
		mlx->tex[i].img_addr = mlx_get_data_addr(mlx->tex[i].img,
												&mlx->tex[i].bpp,
												&mlx->tex[i].line_lenght,
												&mlx->tex[i].endian);
		i++;
	}
	return ;
